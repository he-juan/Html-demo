			...
			if (typeof selector === "string") {
				...
				if (match && (match[1] || !context)) {
					...
					if (match[1]) {
						...
						// HANDLE: $(html, props)
						// jQuery.isPlainObject(context)判断传进来的context是否是对象字面量的形式
						// (/^<([a-z][^\/\0>:\x20\t\r\n\f]*)[\x20\t\r\n\f]*\/?>(?:<\/\1>|)$/i)
					...
					// HANDLE: $(expr, $(...))
					// 没传上下文，或者传了判断context是否为jq对象
					// $('div')  -->{0:div,length:1} $('li',$('ul'))
	...
	function augmentWidthOrHeight(elem, name, extra, isBorderBox, styles) {
		...
		for (; i < 4; i += 2) {
			...
			if (isBorderBox) {
				...
				// border-box includes padding, so remove it if we want contents
				if (extra === "content") {
				...
				// At this point, extra isn't contents, so add padding
				val += jQuery.css(elem, "padding" + cssExpand[i], true, styles);

				// At this point, extra isn't contents nor padding, so add border
				if (extra !== "padding") {
					val += jQuery.css(elem, "border" + cssExpand[i] + "Width", true, styles);
				}
			}
